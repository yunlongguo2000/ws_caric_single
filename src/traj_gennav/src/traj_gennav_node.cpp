...
{
  ros::init(argc, argv, "traj_gennav"); //Initialize ROS node
...
{
  // A callback function that subscribes to odometry information. 
  // It converts odometry data into vectors and quaternions in the Eigen library for subsequent trajectory generation.
  got_odom_ = true; // Indicates that odometer information has been received
  current_odom_.timestamp = ros::Time::now(); // Record the current time
  geoPt3toEigenVec3(msg->pose.pose.position, current_odom_.pos); // Convert the position in the message to a three-dimensional vector in the Eigen library and store it in current_odom_.pos

  if (sim_type_ != "rotors")
  { 
    // If sim_type_ is not "rotors", the linear velocity is directly converted to a 3D vector of Eigen and stored.
    // when velocity from odom is in ENU frame
    geoVec3toEigenVec3(msg->twist.twist.linear, current_odom_.vel);
  }
  else
  { 
    // If sim_type_ is "rotors", it means that the velocity is expressed in the body coordinate system and needs to be converted to the velocity vector in the world coordinate system.
    // when velocity from odom is in body frame
  ...
  // Get the direction cosine matrix(DCM)
  ...
  {
    // If sim_type_ is "none_st" or "sim_st", the quaternion is further processed to obtain the Euler angles and transform them, and finally obtain the DCM again
    Eigen::Vector3d _rpy;
...
{ 
  // Processes the received wall model message
  // Its main function is to convert the received wall model data into the world coordinate system and update the relevant internal state
  ROS_INFO_ONCE("Got first wall model message!");
  // Plane model equation: Ax + By + Cz + D = 0 ??
...
{ 
  // Control whether to start publishing tracks based on the received Boolean message
  if (msg->data != start_command_prev_)
  {
    if (msg->data == true && trajectory_obtained_)
    { // command to start and got traj already
      start_trajectory_ = true;
      idle_state_ = false;
      ROS_INFO("STARTING TRAJECTORY PUBLISH!");
    }
    else if (msg->data == false)
    { // command change to stop traj
...
{ 
  // Processing received path messages
  // Generate a trajectory based on the received path message and perform some related initialization and processing
  if (!path_from_message_)
  { 
    // Check whether getting the path from the message is allowed
    // If the command received is different from the previous command, enter the conditional statement
  ...
  // Initialization
  start_trajectory_ = false; // Stop publishing the trajectory
  idle_state_ = true; // Enter the idle state
  trajectory_wip_ = false;
  initial_waypoints_.clear(); // Clear the initial waypoint list
  addCurrentOdometryWaypoint(); // Add the current odometry position as the waypoint
  ...
  // }
  // Set the heading of the waypoint according to different modes
  for (size_t i = 1; i < initial_waypoints_.size(); i++)
  {
    if (heading_mode_ == "manual")
    { 
      // Extract the quaternion of the previous waypoint from the message
      ...
      tf::Matrix3x3 mm(_tmp); // Convert quaternion to rotation matrix
      double _roll, _pitch, _yaw;
      mm.getRPY(_roll, _pitch, _yaw);
      initial_waypoints_[i].yaw = _yaw; // Assign the extracted yaw angle to the yaw of the current path point
    }
    else if (heading_mode_ == "auto")
    {
      // Compute heading in direction towards next point.
      // Calculate the heading angle of the current waypoint relative to the previous waypoint
...
trajectory_msgs::MultiDOFJointTrajectory generateTrajOnline(double timeinTraj, double T_s, int horizon)
{
  ...
  for (int n = 0; n < horizon; n++)
  {
    ...
    { // find which segment it is at and publish accordingly
    ...
    else if ((planeWorldABC_Est_ - tmp_vec).norm() <= 0.0001)
    { // no update in wall coefficients at all
      _pos_reprojected = _pos;
    }
    else
    {
      _pos_reprojected = _pos + planeWorldABC_Est_ * (-planeWorldD_Est_ + desired_distance_s_ - planeWorldABC_Est_.dot(_pos));
    }
...
Eigen::VectorXd timeAllocation(Eigen::MatrixXd Path)
{
  ...
  // Calculate the time and distance required to accelerate to maximum speed
  float time_to_max_v = reference_speed_ / reference_acceleration_; // t = v/a
  float dist_to_max_v = 0.5 * reference_acceleration_ * time_to_max_v * time_to_max_v; // s = 0.5*a*t^2
  ...
  { 
    ...
    // Determine whether the distance is greater than twice dist_to_max_v
    if (distance > dist_to_max_v * 2)
    { 
      // The time is the total time of accelerating to the maximum speed, 
      // driving at a constant speed, 
      // and decelerating to a standstill
      time(i) = 2 * time_to_max_v + (distance - dist_to_max_v * 2) / reference_speed_;
    }
    else
    { 
      // Time is the total time of acceleration and deceleration
